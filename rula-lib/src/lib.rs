...
pub mod message {
    ...
    pub fn __static__Message(
        kind: &str,
        src_qnic: QnicInterface,
        dst_qnic: QnicInterface,
    ) -> Message {
        Message::new(
            kind,
            &src_qnic,
...
pub mod qnic {
    ...
    impl QnicInterface {
        ...
        pub fn __static__get_partner_by_hop(&mut self, distance: u64) -> QnicInterface {
            self.routing_table.get(&distance).unwrap().clone()
        }
    }
}
pub mod qubit {
